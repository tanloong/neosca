...
from neosca.ns_sca.ns_sca_counter import NsSCACounter, NsSCAStructure
...
class TestStructure(BaseTmpl):
    def test_init(self):
        self.assertRaises(ValueError, NsSCAStructure, **{"name": "S", "description": "sentence"})
        self.assertRaises(
            ValueError,
            NsSCAStructure,
            **{
...
class TestStructureCounter(BaseTmpl):
    def test_init(self):
        ...
        self.assertRaises(ValueError, NsSCACounter, **kwargs_duplicated_defs)

        kwargs_undefined_measures = {"selected_measures": ["an_undefined_measure"]}
        self.assertRaises(ValueError, NsSCACounter, **kwargs_undefined_measures)
        ...
        counter = NsSCACounter(**kwargs_with_user_defs_without_selected_measures)
        self.assertEqual(counter.selected_measures, [*NsSCACounter.DEFAULT_MEASURES, "A", "B"])
        ...
        counter = NsSCACounter(**kwargs_with_user_defs_with_selected_measures)
        self.assertEqual(
    ...
    def test_undefined_measure(self):
        NsSCACounter.check_undefined_measure(NsSCACounter.DEFAULT_MEASURES, None)
        NsSCACounter.check_undefined_measure(["VP", "CT/T"], {"A"})
        self.assertRaises(ValueError, NsSCACounter.check_undefined_measure, ["VP", "CT/T", "NULL"], None)
        NsSCACounter.check_undefined_measure(["VP", "CT/T", "A"], {"A"})

    def test_check_duplicated_def(self):
        user_structure_defs = [{"name": "A"}, {"name": "A"}]
        self.assertRaises(ValueError, NsSCACounter.check_user_structure_def, user_structure_defs)
        ...
        user_defined_snames = NsSCACounter.check_user_structure_def(user_structure_defs)
        self.assertEqual(user_defined_snames, {"A", "B"})

    def test_get_structure(self):
        counter = NsSCACounter()
        for sname in NsSCACounter.DEFAULT_MEASURES:
    ...
    def test_get_all_values(self):
        ...
        counter = NsSCACounter(selected_measures=selected_measures, user_structure_defs=user_structure_defs)
        self.assertTrue("Filename", value_dict.keys() - set(counter.selected_measures))
    ...
    def test_add(self):
        ...
        counter1 = NsSCACounter(selected_measures=selected_measures1)
        counter2 = NsSCACounter(selected_measures=selected_measures2)
