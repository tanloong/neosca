...
from .ns_settings.ns_dialog_settings import NsDialogSettings
...
from .ns_widgets.ns_dialogs import (
    NsDialogAbout,
    NsDialogTableAcknowledgments,
    NsDialogTableCache,
    NsDialogTextEditCiting,
)
from .ns_widgets.ns_table_file import NsTableFile
from .ns_widgets.ns_widget_lca import NsWidgetLCA
from .ns_widgets.ns_widget_sca import NsWidgetSCA
from .ns_widgets.ns_widgets import NsMessageboxQuestion


class NsMainGUI(QMainWindow):
    def __init__(self, *args, with_button_pdb: bool = False, **kwargs):
        ...
        qss += f"""\n* {{
         font-family: {NsSettings.value("Appearance/font-family")};
         font-size: {NsSettings.value("Appearance/font-size")}pt;
         }}"""
    ...
    def setup_statusbar(self) -> None:
        height_pt = NsSettings.value("Appearance/font-size", type=int)
        height_px = int(pt2px(height_pt, offset=2))
        self.statusBar().setFixedHeight(height_px)

        self.statusBar().setVisible(NsSettings.value("show-statusbar", type=bool))
    ...
    def setup_menu(self):
        ...
        self.action_toggle_status_bar.setChecked(NsSettings.value("show-statusbar", type=bool))
        self.statusBar().setVisible(NsSettings.value("show-statusbar", type=bool))
    ...
    def menu_files_open_folder(self):
        folder_path = QFileDialog.getExistingDirectory(
            parent=self, caption="Open Folder", directory=NsSettings.value("Import/default-path")
        )
        if not folder_path:
            return

        is_recursive = NsSettings.value("Import/include-files-in-subfolders", type=bool)
        file_paths = NsIO.find_files(folder_path, is_recursive)
    ...
    def menu_files_open_file(self):
        file_paths, _ = QFileDialog.getOpenFileNames(
            parent=self,
            caption="Open Files",
            directory=NsSettings.value("Import/default-path"),
            filter=";;".join(available_import_types),
            initialFilter=NsSettings.value("Import/default-type"),
        )
        if not file_paths:
            return
        self.table_file.add_file_paths(file_paths)
    ...
    def menu_files_clear_cache(self):
        if any(NsCache.yield_cname_cpath_csize_fpath()):
            NsDialogTableCache(self).open()
        else:
    ...
    def menu_prefs_settings(self) -> None:
        NsDialogSettings(self).open()

    def menu_prefs_enlarge_font(self) -> None:
        key = "Appearance/font-size"
        point_size = NsSettings.value(key, type=int) + 1
        max_size = NsSettings.value("Appearance/font-size-max", type=int)
        if point_size <= max_size:
            NsQSS.update(self, {"*": {"font-size": f"{point_size}pt"}})
            NsSettings.setValue(key, point_size)
    ...
    def menu_prefs_shrink_font(self) -> None:
        key = "Appearance/font-size"
        point_size = NsSettings.value(key, type=int) - 1
        min_size = NsSettings.value("Appearance/font-size-min", type=int)
    ...
    def menu_prefs_default_font_size(self) -> None:
        key = "Appearance/font-size"
        curr_size = NsSettings.value(key, type=int)
        if curr_size != DEFAULT_FONT_SIZE:
            NsQSS.update(self, {"*": {"font-size": f"{DEFAULT_FONT_SIZE}pt"}})
            NsSettings.setValue(key, DEFAULT_FONT_SIZE)
    ...
    def setup_main_window(self):
        self.widget_sca = NsWidgetSCA(with_button_pdb=self.with_button_pdb)
        self.widget_lca = NsWidgetLCA()
        self.table_file = NsTableFile(self)
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        if not NsSettings.value(key, type=bool):
            ...
            checkbox_exit.stateChanged.connect(lambda: NsSettings.setValue(key, checkbox_exit.isChecked()))
        ...
        for splitter in (self.splitter_central_widget,):
            NsSettings.setValue(splitter.objectName(), splitter.saveState())
        NsSettings.sync()
...
def main_gui():
    ui_scaling = NsSettings.value("Appearance/scaling")
    ...
    ns_window = NsMainGUI(with_button_pdb=False)
