...
class NsWidgetSettingsMisc(NsWidgetSettingsAbstract):
    ...
    def load_settings_cache(self) -> None:
        self.checkbox_cache.setChecked(NsSettings.value(f"{self.name}/cache", type=bool))
        self.checkbox_use_cache.setChecked(NsSettings.value(f"{self.name}/use-cache", type=bool))

    def load_settings_warning(self) -> None:
        self.checkbox_dont_warn_on_exit.setChecked(
            NsSettings.value(f"{self.name}/dont-warn-on-exit", type=bool)
        )
        self.checkbox_dont_warn_on_cache_deletion.setChecked(
            NsSettings.value(f"{self.name}/dont-warn-on-cache-deletion", type=bool)
        )
    ...
    def apply_settings_cache(self) -> None:
        NsSettings.setValue(f"{self.name}/cache", self.checkbox_cache.isChecked())
        NsSettings.setValue(f"{self.name}/use-cache", self.checkbox_use_cache.isChecked())

    def apply_settings_warning(self) -> None:
        NsSettings.setValue(f"{self.name}/dont-warn-on-exit", self.checkbox_dont_warn_on_exit.isChecked())
        NsSettings.setValue(
